 ...
 */
export const mapUserAndEventsToDoneEvents = (
  user: User,
  events: Event[],
): number[] | undefined => {
  if (user !== undefined && user.events !== undefined) {
    ...
    return user.events
      .filter((dv: DoneEvent) => eventIDs.includes(dv.eventID))
      .map((dv: DoneEvent) => dv.eventID);
  } else {
    return undefined;
  }
};

export const mapUserDoneEventsToEvents = (
